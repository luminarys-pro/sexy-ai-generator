...
def get_model_response(prompt_text):
    try:
        ...
        if response.text:
            ...
            st.error("La IA se negó a generar contenido para esta combinación, probablemente debido a sus filtros de seguridad. Por favor, intenta con una combinación de etiquetas diferente o menos explícita.")
            return None
    except Exception as e:
        st.error(f"Error en la comunicación con la IA o en la interpretación del JSON: {e}")
        return None
...
with st.sidebar:
    # ... (La lógica del sidebar no cambia y es estable)
...
with tab_dm:
    ...
    st.info("El Asistente de DMs, con todas las funciones avanzadas (idioma de entrada, variantes, etc.), se implementará en la próxima actualización, una vez que la base de la app sea 100% estable.", icon="🚀")