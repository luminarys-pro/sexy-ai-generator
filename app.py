# ---------------- streamlit_app.py (Versión de Producción Final y Robusta) ----------------
...
def get_model_response(prompt_text):
    try:
        ...
        # CÓDIGO DEFENSIVO: Comprueba si la respuesta tiene texto antes de procesar
        if hasattr(response, 'text') and response.text:
            raw_text = response.text.strip().replace("```json", "").replace("```", "")
            return json.loads(raw_text)
        else:
            st.warning("⚠️ La IA ha bloqueado esta combinación de etiquetas, probablemente por sus filtros de seguridad. Intenta quitar o reemplazar la etiqueta más explícita y vuelve a generar.", icon="🤖")
            return None
...
tab_desc, tab_dm = st.tabs(["📝 **Generador de Descripciones**", "💬 **Asistente de DMs (Próximamente)**"])
...
with tab_dm:
    st.header("Próximamente: El Asistente de DMs Definitivo")
    st.success("Ahora que la base de la aplicación es 100% estable, la siguiente gran actualización será la implementación del Asistente de DMs con todas las funciones avanzadas que hemos planeado. ¡Gracias por tu paciencia!", icon="✅")