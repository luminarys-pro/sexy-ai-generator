...
# ==================== FUNCIÓN DE LÓGICA CENTRAL ROBUSTA ====================
def get_model_response(prompt_text):
    """
    Función robusta para llamar a la API de Gemini y manejar errores de forma segura.
    """
    try:
        ...
        if hasattr(response, 'text') and response.text:
            ...
            # Si la IA devuelve una respuesta vacía, informa al usuario.
            st.warning("⚠️ La IA no generó una respuesta. Esto puede ocurrir debido a los filtros de seguridad para la combinación de etiquetas seleccionada. Por favor, intenta con una combinación diferente o menos explícita.", icon="🤖")
            return None
    except json.JSONDecodeError:
        # Si la respuesta no es un JSON válido, informa y muestra la respuesta cruda.
        st.error("La IA devolvió un formato de texto inesperado y no se pudo interpretar.")
        if 'response' in locals() and hasattr(response, 'text'):
            st.code(response.text, language="text")
        return None
    except Exception as e:
        # Para cualquier otro error de la API.
        st.error(f"Ocurrió un error en la comunicación con la IA: {e}")
        return None

# ==================== PÁGINA PRINCIPAL ====================
st.title("💌 AI Content Assistant")
st.markdown("by **Luminarys Production**")
st.header("Generador de Descripciones para Posts")
...
with desc_col2:
    ...
    for item in st.session_state.last_generation:
