# ---------------- streamlit_app.py (Versión 12.0 - FINAL Y COMPLETA) ----------------
...
with tab_desc:
    ...
    # --- Columna de controles para descripciones
    ...
    with desc_col1:
        ...
        if st.button("🚀 Generar Descripciones", key="gen_desc", use_container_width=True):
            if len(desc_selected_tags) < 2:
                ...
                prompt = f"""
                **Tu Identidad y Rol:** {persona_clause} Tu personalidad debe ser `{desc_intensity}`. Actúas desde la perspectiva de una persona definida por las etiquetas: `{", ".join(desc_selected_tags)}`. Si se especifican características físicas (`{desc_physical_features or 'No especificadas'}`), incorpóralas de forma auténtica.
                **{task_description}**
                **Manual de Estilo:** Debes seguir la filosofía de 'Mostrar, no Decir', ser original y evitar clichés y hashtags. La versión en inglés debe ser una adaptación coloquial.
                **Formato de Salida:** JSON estricto con clave "messages", conteniendo una lista de ideas. Cada idea tiene "id" y una lista "outputs" para cada idioma (`{", ".join(desc_output_languages)}`).
                """
    ...
    # --- Columna de resultados para descripciones
    with desc_col2:
    ...
    if not st.session_state.dm_conversation_history:
        ...
        with st.form("reply_form"):
            ...
            if submitted and fan_reply:
                ...
                reply_prompt = f"Tu Rol: {context['persona']}\nObjetivo de la Conversación: {context['scenario']}\nHistorial: {history_for_prompt}\nTu Misión: Lee el último mensaje del fan y genera 3 opciones de respuesta distintas y creativas para continuar la conversación y guiarla hacia el objetivo. Formato: JSON con clave 'suggestions' y una lista de 3 strings."
